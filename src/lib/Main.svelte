<script>
  ...
  async function getPdfInfo(blob) {
    if (!browser || !pdfjsLib) {
      return { type: 'pdf', numPages: 0, thumbnails: [] };
    }
    ...
    const thumbnails = await Promise.all(
      Array.from({ length: Math.min(6, numPages) }, async (_, i) => {
    ...
    return { type: 'pdf', numPages, thumbnails };
  }
            ...
            {#if message.FileAttached}
              {#if message.type === 'pdf'}
                <div class="pdf-preview">
                  {#each message.thumbnails as thumbnail, i}
                    <img src={thumbnail} alt={`Page ${i + 1}`} />
                  {/each}
                  {#if message.numPages > 6}
                    <span class="more-pages">...</span>
                    <p>Please check the original file</p>
                  {/if}
                </div>
                ...
                <div class="audio-message">
                  ...
                  {#if message.AudioTranscription}
                    <div class="transcription">
                      ...
                      <button class="edit-transcription" on:click={() => editTranscription(message)}>
